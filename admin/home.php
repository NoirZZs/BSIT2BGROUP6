...
if(isset($_POST['reset_bookings'])) {
    // Check if $con is set and not null before using it
    if(isset($con) && !is_null($con)) {
        $delete_sql = "TRUNCATE TABLE roombook"; // Assuming 'roombook' is the table name for room bookings
        if(mysqli_query($con, $delete_sql)) {
            echo '<script>alert("All room bookings cleared."); window.location.href = "home.php";</script>';
            exit();
        } else {
            echo '<script>alert("Failed to clear room bookings: ' . mysqli_error($con) . '")</script>';
        }
    } else {
        echo '<script>alert("Database connection is not available.")</script>';
    }
}
?> 
<!DOCTYPE html>
