            ...
            <div id="page-inner">
                ...
                <div class="row">
                    ...
                    <div class="panel panel-info">
						...
						<form method="post">
						<input type="hidden" name="booking_id" value="<?php echo $id; ?>"> <!-- Add hidden input field to store the booking ID -->
...
<?php
						if(isset($_POST['co'])) {
							...
							if($st == "Confirm") {
								$id = $_POST['booking_id']; // Make sure to fetch the booking ID
								
								// Update status in roombook table
								$urb = "UPDATE `roombook` SET `stat`='$st' WHERE id = '$id'";
								if(mysqli_query($con, $urb)) {
									if($f1 != "NO" && $troom == "VIP Room") {
										$sqlUpdate = "UPDATE `room` SET `place` = 'NotFree', `cusid` = '$id' WHERE `bedding` = '$bed' AND `type` = 'VIP Room'";
										mysqli_query($con, $sqlUpdate);
									} elseif ($f2 != "NO" && $troom == "Deluxe Room") {
										$sqlUpdate = "UPDATE `room` SET `place` = 'NotFree', `cusid` = '$id' WHERE `bedding` = '$bed' AND `type` = 'Luxury Room'";
										mysqli_query($con, $sqlUpdate);
									} elseif ($f3 != "NO" && $troom == "Luxury Room") {
										$sqlUpdate = "UPDATE `room` SET `place` = 'NotFree', `cusid` = '$id' WHERE `bedding` = '$bed' AND `type` = 'Boutique Room'";
										mysqli_query($con, $sqlUpdate);
									} elseif ($f4 != "NO" && $troom == "Boutique Room") {
										$sqlUpdate = "UPDATE `room` SET `place` = 'NotFree', `cusid` = '$id' WHERE `bedding` = '$bed' AND `type` = 'Deluxe Room'";
										mysqli_query($con, $sqlUpdate);
									}
												 $type_of_room = 0;       
														if($troom=="VIP Room")
														...
														if(mysqli_query($con, $psql)) {
															// Update room status in room table
															$notfree = "NotFree";
